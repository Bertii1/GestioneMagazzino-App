...
function ServerSetupScreen({
  ...
  const handleConnect = async () => {
    const h = host.trim();
    if (!h) {
      Alert.alert("Inserisci l'indirizzo del server");
      return;
    }
    setConnecting(true);
    ...
    const portSuffix =
      p && p !== defaultPort ? `:${p}` : useHttps ? "" : ":3000";
  ...
  return (
    <View style={ss.container}>
      <View style={ss.card}>
        ...
        >
          <TouchableOpacity
            style={{
              flex: 1,
              paddingVertical: 10,
              backgroundColor: useHttps ? "#2563EB" : "#F3F4F6",
              alignItems: "center",
            }}
            onPress={() => {
              setUseHttps(true);
        ...
        <Text style={ss.label}>
          {useHttps ? "Dominio o IP del server" : "Indirizzo IP del server"}
        </Text>
        <TextInput
          style={ss.input}
          value={host}
...
export default function AppNavigator() {
  const { serverUrl, isDiscovering, progress, discover, setManualUrl } =
    useServerStore();
  const { isAuthenticated, isLoading, restoreSession, mustChangePassword } =
    useAuthStore();
        ...
        {!isAuthenticated ? (
          ...
          <>
            ...
            <Stack.Screen
              ...
              options={{ title: "Scaffale" }}
            />
            <Stack.Screen
              name="ShelfQR"
              component={ShelfQRScreen}
              options={({ route }) => ({
                title: `QR · ${route.params.shelfCode} · R${route.params.level}`,
              })}
            />
            <Stack.Screen
              name="ProductDetail"
            ...
            <Stack.Screen
              ...
              options={({ route }) => ({
                title: `Stampa QR · ${route.params.warehouseName}`,
              })}
            />
            <Stack.Screen
              name="MyQRCode"
